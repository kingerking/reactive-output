 ...
 * see planning/docs-general/ElementClass.md for further information.
 * 
 * @author Kyle King
 */
export default class Element extends EventEmitter {
    ...
    _prevProps = {};
    /**
     * number of times this Element has rendered.
     */
    drawCount = 0;
    _props = {};
    _renderer = null; // elements default renderer if no custom one is passed.
    // array of previous lines rendered.
    // warning: once you start editing lines in a action then this array will be reset.
    previousLines = [];
    ...
    render = (use_renderer = this.renderer) => {
        ...
        const action = funct((error) => { 
            console.log('after render, incrementing drawCount, error: ', error);
            this.drawCount++;
        });
