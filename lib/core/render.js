 ...
 */
render.action = (element, callback = (err) => { }) => (operations) => {
    ...
    return {
        ...
        execute: () => {
            ...
            _.forEach(operations, (operation) => {
                operation(err => { err = true; callback(err) });
            });
            if (!err)
                callback();
        },
 ...
 * types / managers of actions happening on a given element therefore we create a separate action
 * creation helpers to combat this and make the process a lot easier. when it comes to flattening the tree
...
render.loop = () => {
    ...
    // clone the queue.
    const activeQueue = [...render.queue];//_.without(render.queue, null, undefined).filter(element => !typeof element == 'number');
    render.queue = [];
